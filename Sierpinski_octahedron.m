function [V, T] = Sierpinski_octahedron(nb_it, option_display)
%% Sierpinski_octahedron : function to compute, display, and save
% a Sierpinski octahedron at any iteration number / depth level.
%
...
% Description
%
% Sierpinski_octahedron(nb_it) computes and display the nb_it
...
%
%
% Example #1 : computes and displays the simple Sierpinski octahedron at iteration 1
%
% Sierpinski_octahedron(1);
%
%
% Example #2 : computes and displays the Sierpinski octahedron at iteration 3
...
if option_display
    ...
    trisurf(T,V(:,1),V(:,2),V(:,3)), shading interp, hold on; % shading interp,
    colormap([0 1 0]);
...
end % Sierpinski_octahedron


%% Create Sierpinski octahedron basis tetrahedron subfunction
function [M, T, C] = Create_Sierpinski_octahedron_basis_tetrahedron(V1, V2, V3, V4, sample_step)
...
%% Sierpinski tetrahedron iterate subfunction
function [V, T] = Sierpinski_tetrahedron_iterate(V1, V2, V3, V4, nb_it, sample_step)

Summit_array = [V1; V2; V3; V4];
[Vertex_array,Triangle_array,Middle_edge_array] = Create_Sierpinski_octahedron_basis_tetrahedron(V1,V2,V3,V4,sample_step);
...
while p ~= nb_it
    ...
    for j = 1:size(Vertex_array,3) % Loop on current nb tetra               
        ...
        [New_vertex_array(:,:,4*(j-1)+i),New_triangle_array(:,:,4*(j-1)+i),New_middle_edge_array(:,:,4*(j-1)+i)] = ...
            Create_Sierpinski_octahedron_basis_tetrahedron(New_summit_array(1,:,4*(j-1)+i),...
